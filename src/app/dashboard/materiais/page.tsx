        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {materiais.map((mat) => (
            <Link
              key={mat.title}
              href={mat.href}
              className="flex flex-col items-center justify-center bg-white dark:bg-[#23283A] rounded-2xl shadow-soft border border-[#E8E9F1] dark:border-[#23283A] p-8 transition-all duration-200 hover:scale-105 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#D4A017] group cursor-pointer"
              tabIndex={0}
            >
              <span className="text-4xl mb-4 group-hover:scale-110 transition-transform">{mat.icon}</span>
              <span className="text-xl font-bold text-[#D4A017] dark:text-[#E8C547] mb-2 text-center">{mat.title}</span>
