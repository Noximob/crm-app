...
/**
 * LOCAÇÃO · A MINUTA — o contrato preenchido, na tela, como sairia da ClickSign.
 *
 * O gestor pediu: "gera contrato fake pra sabermos como seria". Este
 * componente pega os dados do contrato e monta o documento inteiro —
 * papel branco, cláusulas, lacunas preenchidas — pra ele VER o produto
 * final antes de existir jurídico e assinatura digital.
 *
 ...
 *   · quando o modelo do Lucas virar template na ClickSign, é ELE que
...
import { fimContrato, fmtData, fmtValor, type ContratoLocacao, type ImovelLocacao } from '@/lib/locacao';
import { btnOuro, btnGhost } from './ui';

const porExtensoMeses = (m: number | null): string => (m ? `${m} (${m === 30 ? 'trinta' : m === 12 ? 'doze' : m === 24 ? 'vinte e quatro' : m === 36 ? 'trinta e seis' : m} ) meses` : '____ meses');
...
export default function MinutaContrato({ c, imovel, onFechar }: {
  c: ContratoLocacao;
    ...
    <div className="rounded-xl overflow-hidden border border-white/15">
      ...
      <div className="flex flex-wrap items-center gap-2 bg-white/[0.04] px-4 py-2.5 no-print">
        ...
        <span className="ml-auto flex gap-2">
          <button onClick={imprimir} className={btnOuro}>🖨 Imprimir / PDF</button>
      ...
      <div id="minuta-print" className="bg-white text-neutral-900 px-6 sm:px-10 py-8 text-[12.5px] leading-relaxed font-serif">
        ...
        <p className="mb-3">
          <b>LOCADOR(A):</b> <Lacuna v={c.locadorNome} largura="min-w-[220px]" />,
          {' '}<Lacuna v={c.locadorEstadoCivil} largura="min-w-[80px]" />, <Lacuna v={c.locadorProfissao} largura="min-w-[100px]" />,
          {' '}portador(a) do CPF nº <Lacuna v={c.locadorDoc} /> e RG nº <Lacuna v={c.locadorRg} largura="min-w-[90px]" />,
          {' '}residente em <Lacuna v={c.locadorEnderecoAtual} largura="min-w-[200px]" />.
        </p>
        <p className="mb-3">
          <b>LOCATÁRIO(A):</b> <Lacuna v={c.locatarioNome} largura="min-w-[220px]" />,
          {' '}<Lacuna v={c.locatarioEstadoCivil} largura="min-w-[80px]" />, <Lacuna v={c.locatarioProfissao} largura="min-w-[100px]" />,
          {' '}portador(a) do CPF nº <Lacuna v={c.locatarioDoc} /> e RG nº <Lacuna v={c.locatarioRg} largura="min-w-[90px]" />,
          {' '}telefone <Lacuna v={c.locatarioTelefone} largura="min-w-[110px]" />, e-mail <Lacuna v={c.locatarioEmail} largura="min-w-[140px]" />.
        </p>
        <p className="mb-5">
        ...
        <p>
          A locação vigorará por <b>{porExtensoMeses(c.prazoMeses)}</b>, com início
          em <Lacuna v={c.inicio ? fmtData(c.inicio) : ''} /> e término
          em <Lacuna v={fimContrato(c) ? fmtData(fimContrato(c)) : ''} />, quando o LOCATÁRIO
        ...
        <p className="mb-2">
          ...
          dia <Lacuna v={c.diaVencimento ? String(c.diaVencimento) : ''} largura="min-w-[36px]" />, pago por
        ...
        <p>
          <b>Parágrafo único.</b> A taxa de condomínio{c.valorCondominio ? <> (atualmente {fmtValor(c.valorCondominio)})</> : null} será
        ...
        <p>
          O aluguel será reajustado anualmente pela variação do índice <b>{c.indiceReajuste || 'IGP-M'}</b>,
        ...
        <p>
          A presente locação é garantida por <b>{c.garantiaTipo || 'seguro-fiança'}</b>
          {c.garantiaNumero ? <>, contrato/apólice nº <b>{c.garantiaNumero}</b></> : null}
          {c.garantiaVigenciaFim ? <>, com vigência até <b>{fmtData(c.garantiaVigenciaFim)}</b> e renovação anual obrigatória
        ...
        <div className="grid grid-cols-2 gap-x-10 gap-y-10 text-center text-[11px]">
          <div><div className="border-t border-neutral-500 pt-1">{c.locadorNome || 'LOCADOR(A)'}</div>Locador(a)</div>
          <div><div className="border-t border-neutral-500 pt-1">{c.locatarioNome || 'LOCATÁRIO(A)'}</div>Locatário(a)</div>
