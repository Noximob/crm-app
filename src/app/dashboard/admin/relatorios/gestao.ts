...
// o campeão, o que trabalha e não converte, e o que sumiu.
// ---------------------------------------------------------------------------
  ...
  perfis.forEach((p) => {
    ...
    p.etapas.forEach((etapa, i) => {
      ...
      atividade.set(l.id, {
        ...
        tarefas: [{ id: `t-${l.id}`, tipo: 'Ligação', status: i % 3 === 0 ? 'pendente' : 'concluída', dueMs: agora - (i % 3 === 0 ? 4 : 10) * DIA, concluidaMs: i % 3 === 0 ? 0 : agora - 9 * DIA }],
      });
    });
  });

  const vendas: VendaGestao[] = [
    { id: 'vd1', corretorUid: 'demo-1', status: 'assinada', dataVenda: '2026-07-10', vgvLiquido: 800_000, comissaoBruta: 40_000, rateio: [{ papel: 'corretor', valor: 18_180 }] },
    { id: 'vd2', corretorUid: 'demo-1', status: 'assinada', dataVenda: '2026-07-22', vgvLiquido: 650_000, comissaoBruta: 32_500, rateio: [{ papel: 'corretor', valor: 14_771 }] },
  ];

  return computeGestao(leads, corretores, vendas, atividade, new Set(perfis.map((p) => p.uid)));
}
