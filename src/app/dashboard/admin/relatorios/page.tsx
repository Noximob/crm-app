      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="flex justify-center py-24">
            <div className="animate-spin rounded-full h-14 w-14 border-4 border-[#D4A017] border-t-transparent" />
          </div>
        ) : (
          <>
            <div className="flex flex-wrap items-center gap-4 mb-6">
              ...
              <span className={`inline-flex items-center px-4 py-2 rounded-full text-base font-medium ${corretorFilter ? 'bg-[#E8C547]/30 text-[#D4A017]' : 'bg-[#D4A017]/20 text-[#D4A017]'}`}>
            ...
            {avisoEmDestaque && (
              <div className="mb-8 p-6 rounded-2xl bg-[#D4A017]/15 dark:bg-[#D4A017]/25 border-2 border-[#D4A017]/40 flex items-start gap-4">
                <MegaphoneIcon className="w-8 h-8 text-[#D4A017] shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-[#D4A017] uppercase tracking-wider">Aviso em destaque</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
              <div className="bg-white dark:bg-[#23283A] rounded-2xl border-2 border-[#E8E9F1] dark:border-[#23283A] p-6 md:p-8 shadow-md">
                ...
                <div className="space-y-3">
                  {ranking.slice(0, 10).map((r, i) => (
                    <div key={r.userId} className={`flex items-center justify-between py-3 px-4 rounded-xl ${i < 3 ? 'bg-[#D4A017]/10 dark:bg-[#D4A017]/20' : 'bg-[#F5F6FA] dark:bg-[#181C23]'}`}>
                      <span className="flex items-center gap-3">
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${i === 0 ? 'bg-amber-500 text-white' : i === 1 ? 'bg-gray-400 text-white' : i === 2 ? 'bg-amber-700 text-white' : 'bg-[#D4A017] text-white'}`}>{i + 1}</span>
                        <span className="text-base md:text-lg font-medium text-[#2E2F38] dark:text-white">{r.nome}</span>
                      </span>
                      <span className="text-xl font-bold text-[#D4A017] tabular-nums">{r.count}</span>
                    </div>
                  ))}
              ...
              <div className="bg-white dark:bg-[#23283A] rounded-2xl border-2 border-[#E8E9F1] dark:border-[#23283A] p-6 md:p-8 shadow-md">
                <h2 className="text-xl font-bold text-[#2E2F38] dark:text-white mb-5 flex items-center gap-3">
                  <ChartIcon className="w-7 h-7 text-[#D4A017]" />
                    ...
                    return (
                      <div key={etapa}>
                        <div className="flex justify-between text-base mb-1.5">
                          ...
                          <span className="font-bold text-[#D4A017] text-lg tabular-nums shrink-0">{qtd}</span>
                        </div>
                        <div className="h-3 bg-[#E8E9F1] dark:bg-[#181C23] rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-[#D4A017] to-[#E8C547] rounded-full transition-all" style={{ width: `${pct}%` }} />
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="bg-white dark:bg-[#23283A] rounded-2xl border-2 border-[#E8E9F1] dark:border-[#23283A] p-6 md:p-8 shadow-md mb-10 overflow-x-auto">
              <h2 className="text-xl font-bold text-[#2E2F38] dark:text-white mb-2 flex items-center gap-3">
                <UsersIcon className="w-7 h-7 text-[#D4A017]" />
              ...
              <table className="w-full text-base">
                ...
                <tbody>
                  {tabelaPorCorretor.map((row, i) => (
                    <tr
                      key={row.id}
                      className={`border-b border-[#E8E9F1] dark:border-[#23283A] ${corretorFilter === row.id ? 'bg-[#D4A017]/10 dark:bg-[#D4A017]/20' : ''}`}
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
              <div className="bg-white dark:bg-[#23283A] rounded-2xl border-2 border-[#E8E9F1] dark:border-[#23283A] p-6 md:p-8 shadow-md">
                <h2 className="text-xl font-bold text-[#2E2F38] dark:text-white mb-5 flex items-center gap-3">
                  <TargetIcon className="w-7 h-7 text-[#D4A017]" />
                  Metas (corporativo)
                </h2>
                {meta ? (
                  <div className="space-y-5">
                    ...
                    <div>
                      <div className="flex justify-between text-base mb-2">
                        ...
                        <span className="text-2xl font-bold text-[#D4A017] tabular-nums">{meta.percentual ?? 0}%</span>
                      </div>
                      <div className="h-4 bg-[#E8E9F1] dark:bg-[#181C23] rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all ${(meta.percentual ?? 0) >= 100 ? 'bg-[#3AC17C]' : 'bg-[#D4A017]'}`}
                    ...
                    .map(([origem, qtd]) => {
                      ...
                      return (
                        <div key={origem} className="flex justify-between items-center py-2">
                          ...
                          <span className="font-semibold text-[#D4A017] tabular-nums shrink-0">{qtd} ({pct}%)</span>
                        </div>
                      );
                    })}
            ...
            <div className="bg-[#D4A017]/15 dark:bg-[#D4A017]/25 border-2 border-[#D4A017]/40 rounded-2xl p-6 text-center">
