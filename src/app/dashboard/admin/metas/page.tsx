...
export default function AdminMetasPage() {
  ...
  return (
    <div className="max-w-xl mx-auto mt-10 bg-white rounded-2xl p-8 shadow-xl border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <input type="date" className="w-full rounded-lg border px-3 py-2 text-gray-800" value={inicio} onChange={e => setInicio(e.target.value)} required />
          </div>
          <div className="flex-1">
            <label className="block text-sm font-medium mb-1 text-gray-700">Fim</label>
            <input type="date" className="w-full rounded-lg border px-3 py-2 text-gray-800" value={fim} onChange={e => setFim(e.target.value)} required />
          </div>
        </div>
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="block text-sm font-medium mb-1 text-gray-700">VGV Estimado (R$)</label>
            <input type="number" className="w-full rounded-lg border px-3 py-2 text-gray-800" value={vgv} onChange={e => setVgv(e.target.value)} required min="0" step="0.01" />
          </div>
          <div className="flex-1">
            <label className="block text-sm font-medium mb-1 text-gray-700">VGV Realizado (R$)</label>
            <input type="number" className="w-full rounded-lg border px-3 py-2 text-gray-800" value={realizado} onChange={e => setRealizado(e.target.value)} required min="0" step="0.01" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700">% Alcançado</label>
          <input type="text" className="w-full rounded-lg border px-3 py-2 bg-gray-100 text-gray-800" value={percentual + '%'} readOnly />
        </div>
