...
export default function AvisosImportantesPage() {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!titulo.trim() || !mensagem.trim() || !userData?.imobiliariaId) return;
    setLoading(true);
    if (editId) {
      await updateDoc(doc(db, 'avisosImportantes', editId), { titulo, mensagem, data: Timestamp.now() });
    } else {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <form onSubmit={handleSalvar} className="bg-white dark:bg-[#23283A] rounded-2xl shadow-soft border border-[#E8E9F1] dark:border-[#23283A] p-6 mb-8">
        ...
        {editId && (
          <button type="button" className="ml-4 text-sm text-[#F45B69] underline" onClick={() => { setEditId(null); setTitulo(''); setMensagem(''); }}>Cancelar edição</button>
        )}
      </form>
