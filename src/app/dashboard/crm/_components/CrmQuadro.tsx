...
/**
 * O QUADRO (kanban) — um só, pras duas carteiras.
 *
 * Colunas = as 6 FASES do funil. Na carteira da REDE entra a sétima, a
 * gaveta de "Interesse futuro" (até 50, com o teto no título): é pra lá que
 * o corretor manda os leads que quer guardar — e é só de lá que eles saem.
 * Quadro é SÓ leitura: quem move é o circuito (e a gaveta, pelo lead).
 */
...
import KanbanColumn from '../andamento/_components/KanbanColumn';
import { Lead } from '@/types';
import LoadingState from '@/components/ui/LoadingState';
import { ORIGEM_FILTER_OPTIONS, ACAO_FILTER_OPTIONS, getOrigemBucket, getCampanhaDoLead, getAcaoBuckets } from './FilterModal';
import { getTaskStatusInfo, TaskStatus } from '@/lib/leadTasks';
import { getDemoLeads } from '@/lib/espelho/demoData';
import { ETAPAS_DO_ADMIN, ETAPA_INTERESSE_FUTURO } from '@/lib/circuito';
import {
  carteiraDoLead, leadGuardado, rotuloOrigem, FASES,
  CARTEIRA_REDE, CAP_INTERESSE_FUTURO, type Carteira,
} from '@/lib/funilVendas';

/** O lead pertence a este quadro? (mesma régua da lista) */
...
// A cor de cada coluna: a da fase; a gaveta é azul "geladeira"
const COR_GAVETA = '#7DD3FC';
const corDaColuna = (coluna: string) => FASES.find((f) => f.rotulo === coluna)?.cor || COR_GAVETA;

// Persistência dos filtros do kanban (própria — NÃO compartilhada com a lista; uma por carteira)
...
export default function CrmQuadro({ carteira }: { carteira: Carteira }) {
    const ehRede = carteira === CARTEIRA_REDE;
    const KANBAN_FILTERS_KEY = `crm-kanban-filtros-v2-${carteira}`;
    ...
    useEffect(() => {
        ...
        // As colunas são as FASES; na rede, a gaveta de Interesse futuro.
        const stageList = colunas;
        const groupByStage = (list: Lead[]) => {
            ...
            for (const lead of list) {
                ...
                if (!daCarteira(lead, carteira)) continue;
                // guardado mora na gaveta, seja qual for a etapa dele
                const stage = leadGuardado(lead) ? ETAPA_INTERESSE_FUTURO : faseDe(etapa);
        ...
        return () => unsubscribe();
    }, [currentUser, isEspelhoDemo, colunas, faseDe, normalizeEtapa, carteira]);
                        ...
                        <div className="flex gap-4 flex-1 min-h-0 overflow-x-auto w-full max-w-full pb-2">
                            {colunas.map((stage) => (
                                <KanbanColumn
                                    key={stage}
                                    id={stage}
                                    // a gaveta mostra o teto no título — o corretor vê quanto ainda cabe
                                    title={stage === ETAPA_INTERESSE_FUTURO
                                        ? `💤 Interesse futuro · ${(leads[stage] || []).length}/${CAP_INTERESSE_FUTURO}`
                                        : stage}
                                    leads={filteredLeads[stage] || []}
                                    corEtapa={corDaColuna(stage)}
                                />
                            ))}
                        </div>
                    )}
