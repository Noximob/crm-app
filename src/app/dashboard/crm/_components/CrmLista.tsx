...
/**
 * A LISTA DO CRM — uma só, pras duas carteiras.
 *
 * O mesmo componente serve o CRM da casa (/dashboard/crm) e o CRM da rede do
 * corretor (/dashboard/crm-rede). O gestor pediu que o segundo fosse
 * EXATAMENTE igual ao primeiro — então é o mesmo código, com uma prop.
 *
 * O que muda por carteira:
 *   · quais leads entram (casa: da imobiliária e não guardados; rede: os do
 *     corretor + os que ele GUARDOU em Interesse futuro);
 *   · a coluna "Interesse futuro" (só na rede — é a gaveta dele, até 50);
 *   · "Ação agora" não existe na rede: agendar ali é opcional.
 *
 * As colunas são as 6 FASES do funil (não as 8 casas do circuito).
 */
...
import FilterModal, { Filters, ORIGEM_FILTER_OPTIONS, ACAO_FILTER_OPTIONS, getOrigemBucket, getCampanhaDoLead, getAcaoBuckets } from './FilterModal';
...
import {
  carteiraDoLead, leadGuardado, contaNaDisciplina, rotuloDaFase, rotuloOrigem,
  CARTEIRA_REDE, CAP_INTERESSE_FUTURO, type Carteira,
} from '@/lib/funilVendas';

/** O lead pertence a esta lista? Casa: da imobiliária e fora da gaveta. Rede: do corretor, ou guardado. */
const daCarteira = (l: { carteira?: unknown; origemTipo?: unknown; guardado?: unknown }, carteira: Carteira) =>
  carteira === CARTEIRA_REDE
    ? (carteiraDoLead(l) === CARTEIRA_REDE || leadGuardado(l))
    : (carteiraDoLead(l) !== CARTEIRA_REDE && !leadGuardado(l));

const ROTULO_FECHADO = rotuloDaFase(ETAPA_FECHADO);
...
const etapaChipClasses = (etapa: string) => {
    if (etapa === ROTULO_FECHADO) return 'bg-[#34D399]/10 border-[#34D399]/35 text-[#34D399]';
...
export default function CrmLista({ carteira }: { carteira: Carteira }) {
    const ehRede = carteira === CARTEIRA_REDE;
    // estado da lista guardado por carteira — filtro da casa não vaza pra rede
    const CRM_LIST_STATE_KEY = `crm-list-state-v2-${carteira}`;
    const { currentUser, isEspelhoDemo } = useAuth();
    const { fasesRotulos, faseDe, normalizeEtapa } = usePipelineStages();
    // As colunas desta lista: as 6 fases; na rede, mais a gaveta.
    const colunas = useMemo(() => (ehRede ? [...fasesRotulos, ETAPA_INTERESSE_FUTURO] : [...fasesRotulos]), [fasesRotulos, ehRede]);
    // A coluna de um lead: guardado mora na gaveta, senão é a fase da etapa dele.
    const colunaDe = (lead: { etapa?: string; guardado?: unknown }) =>
        leadGuardado(lead) ? ETAPA_INTERESSE_FUTURO : faseDe(lead.etapa);
    ...
    // Se a coluna filtrada não existe nesta carteira (ex.: gaveta só na rede), limpa o filtro.
    useEffect(() => {
    ...
    useEffect(() => {
        if (isEspelhoDemo) {
            ...
            setLeads(getDemoLeads()
                ...
                .filter(l => daCarteira(l as any, carteira))
                .map(l => {
                    const pendentes = ((l as any).tasks || []).filter((t: any) => t.status === 'pendente');
                    return {
                        ...l,
                        pendentes,
                        taskStatus: statusDoLead((l as any).etapa, pendentes, contaNaDisciplina(l as any)),
                    };
                }) as unknown as Lead[]);
            setLoading(false);
            return;
    ...
    }, [currentUser, isEspelhoDemo, carteira]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, async (snapshot) => {
            ...
            if (!daCarteira(leadData, carteira)) return;
            ...
            leadData.taskStatus = statusDoLead(leadData.etapa, leadData.pendentes as any, contaNaDisciplina(leadData));
    ...
    const fetchLeads = async () => {
        ...
        try {
            ...
            // Bolsão/Descartado são da área do admin — não aparecem pro corretor
              ...
              // cada lista mostra só a sua carteira (a gaveta mora na rede)
              .filter(lead => daCarteira(lead, carteira));
    ...
    const filteredLeads = useMemo(() => {
        ...
        if (activeFilter) {
            leadsToFilter = leadsToFilter.filter(lead => colunaDe(lead) === activeFilter);
        }
        ...
        if (hasAdvancedFilters) {
            leadsToFilter = leadsToFilter.filter(lead => {
                return Object.entries(advancedFilters).every(([key, selectedOptions]: [string, string[]]) => {
                    ...
                    // Etapa usa a COLUNA do quadro (fase do funil, ou a gaveta) —
                    // senão o filtro completo discordaria do kanban.
    ...
    }, [leads, searchTerm, activeFilter, activeTaskFilter, activeAcaoFilter, activeOrigemFilter, activePropagandaFilter, advancedFilters, faseDe]);
            ...
            <main className="flex flex-col flex-1 min-h-0 gap-2 mt-2">
                ...
                <div className="al-card relative overflow-hidden flex flex-col flex-1 min-h-0 p-3 rounded-2xl">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 flex-shrink-0 min-w-0">
                        <SectionTitle>{ehRede ? 'Minha rede' : 'Gestão de Leads'}</SectionTitle>
                        {ehRede && (
                            <span className="text-[11px] font-bold text-[#7DD3FC] bg-[#7DD3FC]/10 border border-[#7DD3FC]/30 rounded-full px-2.5 py-1 tabular-nums" title="Leads guardados em Interesse futuro — a gaveta tem teto">
                                💤 {leads.filter(l => leadGuardado(l)).length}/{CAP_INTERESSE_FUTURO} guardados
                            </span>
                        )}
                        ...
                        <div className="relative flex-shrink-0" ref={filtroRapidoRef}>
                            ...
                            {filtroRapidoOpen && (
                                <div key={`filtro-etapas-${colunas.join('-')}`} className="absolute left-0 top-full mt-1.5 z-50 w-[min(90vw,420px)] max-h-[70vh] overflow-y-auto rounded-xl border border-white/10 bg-[#12101a] shadow-[0_24px_80px_-24px_rgba(0,0,0,0.9)] py-3 px-3">
                                ...
                                {!loading && paginatedLeads.map((lead) => (
                                    <tr key={lead.id} className="border-b border-white/[0.05] last:border-b-0 hover:bg-white/[0.04] transition-colors">
                                        ...
                                        <td className="px-3 py-1.5 text-xs w-1/5">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider border truncate max-w-[150px] ${etapaChipClasses(colunaDe(lead))}`} title={normalizeEtapa(lead.etapa)}>{colunaDe(lead)}</span>
                                        </td>
                                        <td className="px-3 py-1.5 text-xs w-1/5">
                                ...
                                {paginatedLeads.map((lead) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 mt-2 min-w-0">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider border truncate max-w-[55%] shrink-0 ${etapaChipClasses(colunaDe(lead))}`}>{colunaDe(lead)}</span>
