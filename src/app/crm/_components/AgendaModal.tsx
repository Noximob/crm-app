...
// Supondo que a interface Lead e o StatusIndicator já existam em algum lugar
// Se não, precisaríamos defini-los aqui.
interface Lead {
  id: string;
  nome: string;
  telefone: string;
...
const TAREFA_STATUS_ORDER = ['Tarefa em atraso', 'Tarefa do Dia', 'Sem tarefa'];
...
export default function AgendaModal({ isOpen, onClose }: AgendaModalProps) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const q = query(
            collection(db, `leads/${currentUser.uid}/leads`),
            where('status', 'in', TAREFA_STATUS_ORDER)
        );
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            ...
            // Ordena os leads de acordo com a ordem definida em TAREFA_STATUS_ORDER
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-2xl relative">
                ...
                <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                    {loading ? (
                        ...
                        <p className="text-gray-600 dark:text-gray-300">Nenhuma tarefa encontrada para hoje ou em atraso.</p>
                    ) : (
                        tasks.map(task => (
